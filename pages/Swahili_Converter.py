...
<div class="conv-masthead">
    ...
    <div class="conv-masthead-desc">Bidirectional conversion between Arabic numerals and standard Swahili numerals (Kiswahili Sanifu).</div>
</div>
...
arabic_presets = [0, 5, 11, 21, 100, 256, 1000, 2024]
other_presets  = ['sifuri', 'tano', 'kumi na moja', 'ishirini na moja', 'mia moja', 'mia mbili hamsini na sita', 'elfu moja', 'elfu mbili na ishirini na nne']
...
if direction == 'Arabic → Swahili':
    ...
    arabic_input = st.text_input('Enter an Arabic numeral', key='arabic_input', placeholder='e.g. 256', label_visibility="collapsed")
    if arabic_input:
        val = arabic_input.strip()
        if val.lstrip('-').isdigit():
            try:
                result = num_to_words(int(val))
                st.markdown(f'''<div class="conv-result-card">
        <div class="conv-result-label">Swahili</div>
    ...
    <div class="conv-input-card">
        ...
        <div class="conv-input-hint">Standard Kiswahili Sanifu, e.g. <em>mia mbili hamsini na sita</em>.</div>
    </div>''', unsafe_allow_html=True)
    swahili_input = st.text_input('Enter a Swahili numeral', key='swahili_input', placeholder='e.g. mia mbili hamsini na sita', label_visibility="collapsed")
    if swahili_input:
        val = swahili_input.strip()
        if bool(val.strip()):
            try:
                result = str(words_to_num(val))
                st.markdown(f'''<div class="conv-result-card">
...
st.markdown('''
<div class="conv-caption">Uses the tarakimu library. Noun-class agreement discussed in the Linguistics section. Algorithm by Yi Zou.</div>
''', unsafe_allow_html=True)

st.markdown('''
<div class="conv-nav-footer">
    <a class="conv-nav-btn active" href="/Swahili_Converter">Swahili Numerals Converter</a>
