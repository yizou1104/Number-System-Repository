...
# ============================================================
# KLINGON (tlhIngan Hol) NUMERAL SYSTEM
# Bidirectional: Arabic ↔ Klingon. Range: 0–999,999,999.
# Sources: Marc Okrand "The Klingon Dictionary" (1985, 1992 addendum)
# ============================================================

# ── Backend ──────────────────────────────────────────────────────────────────

_DIGITS = {0:"pagh", 1:"wa'", 2:"cha'", 3:"wej", 4:"loS",
           5:"vagh", 6:"jav", 7:"Soch", 8:"chorgh", 9:"Hut"}
_DIGITS_REV = {v: k for k, v in _DIGITS.items()}

_MAGNITUDES = [(1_000_000, "'uy'"), (100_000, "bIp"), (10_000, "netlh"),
               (1_000, "SaD"), (100, "vatlh"), (10, "maH")]
_MAG_VALUES = {suf: val for val, suf in _MAGNITUDES}


def number_to_klingon(n):
    ...
    if rem >= 1_000_000:
        ...
        # Render the millions multiplier and append 'uy' to its last token
        rendered = number_to_klingon(m)
        toks = rendered.split()
        toks[-1] = toks[-1] + "'uy'"
        parts.append(" ".join(toks))

    for value, suffix in [(100_000, "bIp"), (10_000, "netlh"),
                          (1_000, "SaD"), (100, "vatlh"), (10, "maH")]:
        if rem >= value:
            d = rem // value; rem %= value
            parts.append(_DIGITS[d] + suffix)
...
def _kl_parse_segment(seg):
    """Parse one space-separated token: digit + optional magnitude suffix."""
    for suf in sorted(_MAG_VALUES.keys(), key=len, reverse=True):
        if seg.endswith(suf):
            digit_part = seg[:-len(suf)]
            if digit_part in _DIGITS_REV:
                return _DIGITS_REV[digit_part], _MAG_VALUES[suf]
    if seg in _DIGITS_REV:
        return _DIGITS_REV[seg], 1
    raise ValueError(f"Unrecognised Klingon token: '{seg}'")


def klingon_to_number(text):
    text = text.strip()
    if not text: raise ValueError("Empty input.")
    if text == "pagh": return 0

    tokens = text.split()
    total = 0

    millions_idx = next((i for i, t in enumerate(tokens) if t.endswith("'uy'")), -1)
    if millions_idx >= 0:
        last = tokens[millions_idx]
        recovered = last[:-len("'uy'")]
        if not recovered: raise ValueError("Empty millions multiplier.")
        mult_text = " ".join(tokens[:millions_idx] + [recovered])
        mult_val = klingon_to_number(mult_text)
        if not 1 <= mult_val <= 999:
            raise ValueError(f"Millions multiplier out of range: {mult_val}")
        total += mult_val * 1_000_000
        tokens = tokens[millions_idx + 1:]

    for tok in tokens:
        d, m = _kl_parse_segment(tok)
        total += d * m
    return total
...
# PAGE
# ============================================================
st.set_page_config(page_title="Klingon Numeral Converter", layout="centered")
apply_global_styles()
...
<div class="conv-masthead">
    ...
    <div class="conv-masthead-title">Klingon Numerals</div>
    <div class="conv-masthead-desc">
        Bidirectional conversion between Arabic numerals and Klingon (tlhIngan Hol) —
        decimal system with magnitude-suffix morphology. Note: orthography is case-sensitive.
    </div>
</div>
""", unsafe_allow_html=True)

st.markdown('<div class="conv-section-label">Conversion Direction</div>', unsafe_allow_html=True)
direction = st.radio("Direction", ["Arabic → Klingon", "Klingon → Arabic"],
                     horizontal=True, label_visibility="collapsed", key="kl_dir")

st.markdown('<div class="conv-section-label">Preset Examples</div>', unsafe_allow_html=True)
arabic_presets = [0, 5, 10, 20, 234, 1000, 100000, 1000000]
kl_presets = ["pagh", "vagh", "wa'maH", "cha'maH",
              "cha'vatlh wejmaH loS", "wa'SaD",
              "wa'bIp", "wa''uy'"]

with st.container(border=True):
    st.markdown('<p class="conv-presets-sublabel">Click a value to load it</p>', unsafe_allow_html=True)
    cols = st.columns(4)
    if direction == "Arabic → Klingon":
        ...
        for i, txt in enumerate(kl_presets):
            if cols[i % 4].button(txt, key=f"p_b_{i}", use_container_width=True):
                st.session_state["kl_input"] = txt
...
if direction == "Arabic → Klingon":
    ...
    arabic_input = st.text_input("Arabic numeral", key="arabic_input",
                                 placeholder="e.g. 234", label_visibility="collapsed")
    if arabic_input:
        if arabic_input.isdigit():
            try:
                result = number_to_klingon(int(arabic_input))
                st.markdown(f'<div class="conv-result-card"><div class="conv-result-label">Klingon numeral</div><div class="conv-result-value">{result}</div></div>',
                            unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class="conv-input-card">
        <div class="conv-input-title">Enter a Klingon numeral</div>
        <div class="conv-input-hint">
            Case-sensitive — e.g. <em>cha'vatlh wejmaH loS</em> (234).
            Note: H, S, D, Q, I are distinct from h, s, d, q, i.
        </div>
    </div>
    """, unsafe_allow_html=True)
    kl_input = st.text_input("Klingon numeral", key="kl_input",
                             placeholder="e.g. cha'vatlh wejmaH loS", label_visibility="collapsed")
    if kl_input:
        try:
            result = str(klingon_to_number(kl_input))
...
<div class="conv-caption">
    Klingon (tlhIngan Hol) was created by linguist Marc Okrand for the Star Trek
    franchise. Numbers are decimal, formed by attaching magnitude suffixes
    (maH, vatlh, SaD, netlh, bIp, 'uy') directly to digit roots. The
    orthography is strictly case-sensitive: capital letters denote distinct
    phonemes. Sources: Okrand <em>The Klingon Dictionary</em> (1985 / 1992 addendum),
    Klingon Language Institute. Algorithm by Yi Zou.
</div>
""", unsafe_allow_html=True)

st.markdown("""
<div class="conv-nav-footer">
    <a class="conv-nav-btn active" href="/Klingon_Converter">Klingon Converter</a>
    <a class="conv-nav-btn" href="/Klingon_Linguistics">Klingon Linguistics →</a>
</div>
""", unsafe_allow_html=True)
home_nav()
